...
describe('PATCH /details/email/resendEmail', () => {
  const endpoint: string = '/api/accounts/details/email/resendEmail';
  ...
  it('should reject the request if the email update request is not found', async () => {
    vi.mocked(authDbHelpers.getAccountIdByAuthSessionId).mockResolvedValueOnce(1);
    vi.mocked(mockConnection.execute).mockResolvedValueOnce([
      [
        {
  ...
  it('should reject the request the request if the failed attempts count has reached the limit', async () => {
    vi.mocked(authDbHelpers.getAccountIdByAuthSessionId).mockResolvedValueOnce(1);
    vi.mocked(mockConnection.execute).mockResolvedValueOnce([
      [
        {
    ...
    expect(res.body).toStrictEqual({
      ...
      resData: { expiryTimestamp: 1.771e12 },
    });
  });

  it('should reject the request the request if the emails sent limit has been reached', async () => {
    vi.mocked(authDbHelpers.getAccountIdByAuthSessionId).mockResolvedValueOnce(1);
    vi.mocked(mockConnection.execute).mockResolvedValueOnce([
      [
        {
  ...
  it('should resolve the request and call both incrementAccountRequestEmailsSent and sendEmailUpdateStartEmailService', async () => {
    vi.mocked(authDbHelpers.getAccountIdByAuthSessionId).mockResolvedValueOnce(1);
    vi.mocked(mockConnection.execute).mockResolvedValueOnce([
      [
        {
...
describe('POST /deletion/start', () => {
  const endpoint: string = '/api/accounts/deletion/start';
  ...
  it('should reject the request if the password provided is incorrect, calling handleIncorrectPassword', async () => {
    vi.mocked(authDbHelpers.getAccountIdByAuthSessionId).mockResolvedValueOnce(1);
    vi.mocked(mockConnection.execute).mockResolvedValueOnce([
      [
        {
  ...
  it('should reject the request if an ongoing deletion request is found', async () => {
  ...
  it('should resolve the request and send the expiry timestamp, calling resetFailedSignInAttempts if failed sign in attempts are above 0', async () => {
    vi.mocked(authDbHelpers.getAccountIdByAuthSessionId).mockResolvedValueOnce(1);
    vi.mocked(mockConnection.execute).mockResolvedValueOnce([
      [
        {
    ...
    expect(emailServices.sendAccountDeletionEmailService).toHaveBeenCalledExactlyOnceWith({
      receiver: 'example@example.com',
      displayName: 'John Doe',
      confirmationCode: expect.any(String),
    });
...
describe('PATCH /deletion/resendEmail', () => {
  const endpoint: string = '/api/accounts/deletion/resendEmail';
  ...
  it('should resolve the request, calling incrementAccountRequestEmailsSent and sendAccountDeletionEmailService', async () => {
    const expiryTimestamp: number = Date.now() + dayMilliseconds;
