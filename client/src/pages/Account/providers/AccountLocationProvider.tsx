...
export default function AccountLocationProvider({
  children,
}: AccountLocationProviderProps): JSX.Element {
  const [accountLocation, setAccountLocation] = useState<AccountLocation>('profile');

  const contextValue: AccountLocationContextType = useMemo(
    () => ({ accountLocation, setAccountLocation }),
    [accountLocation]
  );
