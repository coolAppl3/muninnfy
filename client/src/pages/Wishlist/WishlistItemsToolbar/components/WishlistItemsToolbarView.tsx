...
export default function WishlistItemsToolbarView(): JSX.Element {
  ...
  return (
    <button
      type='button'
      title={`Switch to ${isSingleColumnView ? 'double' : 'single'} column grid`}
      aria-label={`Switch to ${isSingleColumnView ? 'double' : 'single'} column grid`}
      ...
      onClick={() => setIsSingleColumnView((prev) => !prev)}
    >
