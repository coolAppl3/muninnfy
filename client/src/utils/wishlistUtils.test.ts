...
describe('getWishlistPrivacyLevelName', () => {
  it('should return Public if the public wishlist privacy level is provided', async () => {
    expect(getWishlistPrivacyLevelName(PUBLIC_WISHLIST_PRIVACY_LEVEL)).toBe('Public');
  });

  it('should return Followers if the followers wishlist privacy level is provided', async () => {
    expect(getWishlistPrivacyLevelName(FOLLOWERS_WISHLIST_PRIVACY_LEVEL)).toBe('Followers');
  });

  it('should return Private if the private wishlist privacy level is provided', async () => {
    expect(getWishlistPrivacyLevelName(PRIVATE_WISHLIST_PRIVACY_LEVEL)).toBe('Private');
  });
});

describe('getFormattedPrice', () => {
  it('should correctly format the provided price, adding two decimal points if it is below 1000, or using umber abbreviations (e.g. K, M) when it is 1000 or above', async () => {
